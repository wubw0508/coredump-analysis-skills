...
fix_version() {
    ...
    local fixable_count=$(cat "$analysis_file" | jq '[.crashes[] | select(.fixable == true)] | length' 2>/dev/null)
    if [[ "$fixable_count" -eq 0 ]]; then
        log "  跳过: 无fixable崩溃"
        return 0
    fi
    
    log "  发现 $fixable_count 个fixable崩溃"
    
    cd "$SCRIPTS_DIR"
    local cmd="python3 auto_fix_submit.py --package $package --version $version --workspace $WORKSPACE --target-branch $TARGET_BRANCH"
...
fix_package() {
    ...
    for version_dir in $version_dirs; do
        ...
        local analysis_file="$version_dir/analysis.json"
        
        if [[ ! -f "$analysis_file" ]]; then
            continue
        fi
        
        if fix_version "$package" "$version" "$analysis_file"; then
            ((success_count++)) || true
        else
            ((fail_count++)) || true
        fi
